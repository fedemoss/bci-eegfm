...
GAIN="${GAIN:-1.0}"
EPOCHS="${EPOCHS:-20}"
BACKBONES="${BACKBONES-cbramod reve_large}"
...
if [ "$GAIN_SWEEP" = "1" ]; then
    ...
    benchopt run "$BENCH" -d "$DATASET" --no-plot \
        -s "EEGFM[backbone=cbramod,arm=probe,gain=0.25,0.5,1.0,2.0,4.0,head=all_patch_reps,avgpool,n_epochs=$EPOCHS]" \
        -o "BCI-decoding[training=True]" 2>&1 | tee "$LOGS/gain_sweep.log"
fi

# --- stages 1 & 2: train each arm, then re-score it with Tent.
for bb in $BACKBONES; do
 # "init" only selects a local CBraMod checkpoint; REVE resolves through the Hub.
 inits="$INITS"; [ "$bb" != "cbramod" ] && inits="pretrained"
 for arm in $ARMS; do
  for init in $inits; do
    ...
    benchopt run "$BENCH" -d "$DATASET" --no-plot \
        -s "EEGFM[backbone=$bb,arm=$arm,init=$init,gain=$GAIN,head=$HEAD,n_epochs=$EPOCHS]" \
    ...
    benchopt run "$BENCH" -d "$DATASET" --no-plot \
        -s "EEGFM[backbone=$bb,arm=$arm,init=$init,gain=$GAIN,head=$HEAD,adapt=tent]" \
        2>&1 | tee "$LOGS/${tag}_tent.log"

    echo "=== ${tag}: + Tent + diversity ($TENT_DIV) ==="
    benchopt run "$BENCH" -d "$DATASET" --no-plot \
        -s "EEGFM[backbone=$bb,arm=$arm,init=$init,gain=$GAIN,head=$HEAD,adapt=tent,tent_diversity=$TENT_DIV]" \
    ...
    if [ "$arm" = "neurottt" ]; then
        ...
        benchopt run "$BENCH" -d "$DATASET" --no-plot \
            -s "EEGFM[backbone=$bb,arm=$arm,init=$init,gain=$GAIN,head=$HEAD,adapt=ssl,ttt_chunk=$TTT_CHUNK]" \
...
echo
echo "=== summary ==="
python "$EEGFM_HOME/run/summarize.py"
